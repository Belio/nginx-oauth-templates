...
if ts.required_params_present({'state'}, params) then
  local state_mode = tostring(params["state"])
  ...
  ngx.log(0,tostring(client_data.client_id))
  ...
  ngx.log(0,state_mode)
  return ngx.redirect(client_data.redirect_uri .. "?code="..code .. "&state=" .. (state_mode or ""))
else
